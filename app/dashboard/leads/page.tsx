...
import { leadSummary } from "@/lib/analytics";
...
export default function LeadsPage() {
  ...
  const summary = useMemo(() => leadSummary(state.leads), [state.leads]);
  ...
  return (
    <DashboardShell title="Leads" description="Turn detailed event requests into booked celebrations." actions={<Button variant="secondary" onClick={exportCsv}><Download size={16} /> Export CSV</Button>}>
      <div className="lead-summary-row"><div><span>{summary.total}</span><small>Total inquiries</small></div><div><span>{summary.newCount}</span><small>New</small></div><div><span>{summary.quotedCount}</span><small>Quotes sent</small></div><div><span>{summary.wonCount}</span><small>Won</small></div>{summary.bookingRate === null ? <p>Your booking rate appears once inquiries are marked <strong>Won</strong> or <strong>Lost</strong>.</p> : <p><strong>{summary.bookingRate}%</strong> of decided inquiries became booked events</p>}</div>
