...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, Globe2, Plus, Rocket, Trash2, Upload } from "lucide-react";
...
export default function OnboardingPage() {
  ...
  const [business, setBusiness] = useState({ ...state.business, serviceAreas: state.business.serviceAreas.join(", ") });
  const [theme, setTheme] = useState({ ...state.theme });
  const [items, setItems] = useState(state.menuItems.slice(0, 3).map(item => ({ ...item })));
  ...
  const publish = () => {
    setState(current => ({
      ...current,
      business: { ...current.business, ...business, serviceAreas: business.serviceAreas.split(",").map(area => area.trim()).filter(Boolean), published: true },
      theme,
      menuItems: [...items, ...current.menuItems.filter(item => !items.some(first => first.id === item.id))],
      onboarded: true,
    }));
    notify("Your website is live");
    router.push("/site/olive-and-ember");
  };
  ...
  return (
    <main className="onboarding-page">
      <header className="onboarding-header"><BrandMark /><div className="onboarding-progress"><span>Step {step} of 5</span><div><i style={{ width: `${completion}%` }} /></div></div><button onClick={() => router.push("/dashboard")}>Save and exit</button></header>
      <div className="onboarding-shell">
        ...
        <section className="onboarding-content">
          ...
          {step === 2 && <div className="onboarding-step wide"><span className="step-kicker">02 · Your visual foundation</span><h1>Choose a template that fits your style.</h1><p>Every template is fully responsive and stays professionally designed as you customize it.</p><div className="onboarding-template-grid">{(["editorial", "modern", "warm"] as TemplateName[]).map(value => <TemplateCard key={value} value={value} selected={theme.template === value} onSelect={() => setTheme({ ...theme, template: value })} />)}</div></div>}
