...
export function LeadDetailsDrawer({ lead, onClose }: { lead: Lead | null; onClose: () => void }) {
  ...
  const updateStatus = (status: LeadStatus) => {
    setState(current => ({ ...current, leads: current.leads.map(entry => entry.id === lead.id ? { ...entry, status } : entry) }));
    void (async () => {
      try {
        const { error } = await createClient().from("leads").update({ status }).eq("business_id", businessId).eq("id", lead.id);
